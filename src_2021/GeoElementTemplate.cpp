...
template<class TGeom>
MElementType GeoElementTemplate<TGeom>::Type() {
    return TGeom::Type();
}
...
template<class TGeom>
void GeoElementTemplate<TGeom>::GradX(const VecDouble &xi, VecDouble &x, MatrixDouble &gradx) {
    int NNodes = this->NNodes();
    MatrixDouble coord(3, NNodes);
...
template<class TGeom>
void GeoElementTemplate<TGeom>::Jacobian(const MatrixDouble &gradx, MatrixDouble &jac, MatrixDouble &axes, double &detjac, MatrixDouble &jacinv) {
